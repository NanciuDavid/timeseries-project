...
y_no_pandemic = pd.concat([y_pre, y_post])

print(f"No-pandemic period: {y_no_pandemic.index.min()} to {y_no_pandemic.index.max()} ({len(y_no_pandemic)} days)")

# SARIMA analysis for no-pandemic data
def run():
    sarima_analysis(y_no_pandemic, 'no_pandemic', plots_dir)
